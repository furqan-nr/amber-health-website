  ...
  return (
    <section id="about" className="py-8 sm:py-12 md:py-16 lg:py-20">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl xl:text-5xl font-black text-gray-900 mb-6 sm:mb-8 md:mb-10 lg:mb-12 text-center lg:text-left">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
          {teamMembers.map((member, index) => (
            <Card
              key={index}
              className="group relative min-h-[280px] sm:min-h-[320px] md:min-h-[360px] lg:min-h-[400px] overflow-hidden rounded-xl border-0 shadow-lg hover-scale transition-transform duration-300"
            >
              {/* Background Image */}
              <div 
              ...
              <CardContent className="relative h-full flex flex-col justify-end p-4 sm:p-6 lg:p-8 text-white z-10">
                <h3 className="text-base sm:text-lg md:text-xl lg:text-2xl font-bold mb-2">{member.name}</h3>
                <p className="text-sm sm:text-base md:text-lg lg:text-xl mb-2 sm:mb-3 lg:mb-4">{member.title}</p>
                <p className="text-xs sm:text-sm opacity-90 leading-relaxed">{member.description}</p>
              </CardContent>
            </Card>
          ))}
        </div>
