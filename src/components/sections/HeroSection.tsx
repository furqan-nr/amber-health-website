    ...
    <section id="home" className="relative py-8 sm:py-12 md:py-16 lg:py-24 xl:py-32 w-full max-w-full overflow-x-hidden">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full max-w-full">
        ...
        <div className="grid lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-12 items-center mb-12 sm:mb-16 lg:mb-20 w-full max-w-full">
          <div className="space-y-4 sm:space-y-6 lg:space-y-8 relative z-20 text-center lg:text-left">
            <h1 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl xl:text-5xl 2xl:text-6xl font-black text-gray-900 leading-tight">
              MENTAL HEALTH & WELLNESS SOLUTIONS FOR MUSIC INDUSTRY.
            </h1>
            <h2 className="text-base sm:text-lg md:text-xl lg:text-2xl font-bold text-gray-900 tracking-wide">
          ...
          <div className="flex justify-center lg:justify-end relative z-20">
            ...
            <div className="hero-circle hero-circle-1 w-20 h-20 bg-yellow-400 rounded-full opacity-30 absolute top-0 right-0"></div>
            <div className="hero-circle hero-circle-2 w-16 h-16 bg-red-400 rounded-full opacity-25 absolute bottom-0 left-0"></div>
            <div className="hero-circle hero-circle-3 w-12 h-12 bg-orange-400 rounded-full opacity-20 absolute top-10 left-10"></div>
          </div>
        </div>

        {/* Services Content */}
        <div className="relative z-20 w-full">
          <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl xl:text-5xl font-black text-gray-900 mb-6 sm:mb-8 md:mb-12 lg:mb-16 text-center">
            OUR SERVICES
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-8 w-full max-w-full">
            {serviceCards.map((card, index) => (
              <Card
                key={index}
                className="group relative min-h-[280px] sm:min-h-[320px] md:min-h-[360px] lg:min-h-[400px] xl:min-h-[450px] overflow-hidden rounded-xl border-0 shadow-lg cursor-pointer hover-scale transition-transform duration-300"
              >
                {/* Background Image */}
                <div 
